...
Found over 2500 Jabber usernames; ASRep Roasting to crack a user's password, login to a Jabber/XMPP room containing credentials for the `OpenFire` service. Using those credentials, a malicious plugin is uploaded, exploiting CVE-2023-32315 to get a shell as `nt authority\system`
...
1. I navigated to the "Plugins" tab in the OpenFire admin console and uploaded the `openfire-management-tool-plugin.jar` file

![img](/assets/img/Jab/13.webp)

2. I went to the "Server" tab then "Server Settings" and then "Management Tool"

![img](/assets/img/Jab/14.webp)

3. I clicked on the "Management Tool" link it asked for password which is "123"

![img](/assets/img/Jab/15.webp)

4. I selected the "System Command" option and ran `whoami` the output confirmed that it's running as `nt authority\system`
